...
object MockAIService {
    ...
    fun generateResponse(userMessage: String, conversationHistory: List<String> = emptyList()): String {
        val lowerMessage = userMessage.lowercase().trim()
        
        // Maintenance/Issues
        if (lowerMessage.contains("maintenance") || lowerMessage.contains("issue") || 
            lowerMessage.contains("problem") || lowerMessage.contains("broken") ||
            lowerMessage.contains("leak") || lowerMessage.contains("not working")) {
        ...
        // Tickets/Status
        if (lowerMessage.contains("ticket") || lowerMessage.contains("status") || 
            lowerMessage.contains("update") || lowerMessage.contains("check")) {
            return getRandomResponse(listOf(
        ...
        // Contractors
        if (lowerMessage.contains("contractor") || lowerMessage.contains("find") || 
            lowerMessage.contains("recommend") || lowerMessage.contains("assign") ||
            lowerMessage.contains("plumber") || lowerMessage.contains("electrician")) {
            return getRandomResponse(listOf(
        ...
        if (lowerMessage.contains("schedule") || lowerMessage.contains("appointment") || 
            lowerMessage.contains("visit") || lowerMessage.contains("when") ||
            lowerMessage.contains("time") || lowerMessage.contains("available")) {
            return getRandomResponse(listOf(
        ...
        // Ratings/Reviews
        if (lowerMessage.contains("rating") || lowerMessage.contains("review") || 
            lowerMessage.contains("feedback") || lowerMessage.contains("rate")) {
            return getRandomResponse(listOf(
        ...
        if (lowerMessage.contains("help") || lowerMessage.contains("what can you do") ||
            lowerMessage.contains("how") || lowerMessage.contains("what") && lowerMessage.length < 20) {
            return getRandomResponse(listOf(
        ...
        // Default responses - varied and helpful
        return getRandomResponse(listOf(
    ...
    fun getTypingDelay(response: String): Long {
        val baseDelay = 800L
        val charDelay = response.length * 20L
        val randomVariation = Random.nextLong(300, 800)
        return (baseDelay + charDelay + randomVariation).coerceIn(800, 3000)
    }
}
